#!/usr/bin/env sh
# Prepare Python venv and install dependencies. On Windows use Git Bash or WSL.
set -eu
...
if [ "$USE_SYSTEM_PYTHON" = "1" ]; then
  ...
  echo "  PYTHONPATH=src $PY -m vpconnect_install.gui_tk"
else
  if [ ! -d .venv ]; then
    "$PY" -m venv .venv
  fi
  ...
  echo "  . .venv/bin/activate   # Windows: .venv\\Scripts\\activate"
  echo "  python -m vpconnect_install --help"
  echo "  python -m vpconnect_install.gui_tk"
fi
