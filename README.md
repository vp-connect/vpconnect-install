...
3. **Bootstrap** (`configure_bootstrap.py`): для каждого из `00_bashinstall.sh` … `03_getconfigure.sh` — загрузка с GitHub (raw), запуск в домашнем каталоге на сервере. Скрипт **00** ставит `bash` и на Linux пакет с **`ss`** (`iproute2` / `iproute`). Сразу **после успешного 00** (и **до 01**) выполняется **предпроверка портов** на сервере: для портов, которые реально понадобятся в этом прогоне (новый SSH, WireGuard, MTProxy включая внутренний **8888**, VPManage), проверяется, что они ещё не слушаются. При конфликте прогон останавливается с понятным логом — освободите порты или измените настройки в CLI/GUI.
